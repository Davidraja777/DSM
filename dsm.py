...
while True:
    ...
    if event == 'View Records':
        # Get the full date from the user
        date_selected = sg.popup_get_date(title="Select Date", no_titlebar=True)
        if date_selected:
            month, day, year = date_selected
            filename = f"records_{month}_{year}.json"
        ...
        try:
            ...
            sg.popup_error(f"No records found for {calendar.month_name[month]} {day} {year}!", keep_on_top=True)
            records = []
        except json.JSONDecodeError:
            sg.popup_error(f"Error reading records for {calendar.month_name[month]} {day} {year}!", keep_on_top=True)
            records = []

        date_str = datetime(year, month, day).strftime("%Y-%m-%d")
        records = [record for record in records if record[0].startswith(date_str)]
